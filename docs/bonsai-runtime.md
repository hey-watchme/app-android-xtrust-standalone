...
- Hugging Face model: `prism-ml/Bonsai-1.7B-gguf`
- 推奨ファイル: `Bonsai-1.7B-Q1_0.gguf`
...
- `Bonsai-1.7B-Q1_0.gguf` を前提にチャット送受信まで接続した
...
- 実機確認では `libggml-cpu-android_armv8.0_1.so` がロードされ、`Bonsai-1.7B-Q1_0.gguf` の初回ロードまで到達する
...
2. `Bonsai-1.7B-Q1_0.gguf` を端末へ配置する
...
- Hugging Face: `https://huggingface.co/prism-ml/Bonsai-1.7B-gguf`
